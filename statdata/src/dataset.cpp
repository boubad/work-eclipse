...
bool DataSet::change_variable_type(const std::string &sId,
		statdata::DataType rtype) {
...
bool DataSet::indiv_data(const std::string &sId,
...
bool DataSet::indiv_data(int indindex,
		std::map<std::string, boost::any> &oMap) {
	oMap.clear();
	if ((indindex < 0) || (indindex >= (int) this->m_inds.size())) {
		return (false);
...
int DataSet::get_variable_index(const std::string &sId) const {
	...
	std::vector<Variable> &vars =
			const_cast<std::vector<Variable> &>(this->m_vars);
	size_t n = vars.size();
	for (size_t i = 0; i < n; ++i) {
		Variable &v = vars[i];
...
Variable *DataSet::add_variable(const std::string &sId,
	...
	auto it = std::find_if(vars.begin(), vars.end(), [&](Variable & v)->bool {
		return (v.id() == sx);
	});
	if (it != vars.end()) {
		pRet = &(*it);
...
int DataSet::get_indiv_index(const std::string &sId) const {
	...
	std::string sx = boost::to_lower_copy(sid);
	std::vector<Individu> &inds =
			const_cast<std::vector<Individu> &>(this->m_inds);
	size_t n = inds.size();
	for (size_t i = 0; i < n; ++i) {
		Individu &v = inds[i];
		std::string sy = boost::trim_copy(v.id());
...
Individu *DataSet::add_indiv(const std::string &sId, const std::string &sName) {
	...
	auto it = std::find_if(inds.begin(), inds.end(), [&](Individu & v)->bool {
		return (v.id() == sx);
	});
	if (it != inds.end()) {
		pRet = &(*it);
