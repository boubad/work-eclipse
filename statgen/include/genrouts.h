...
namespace statdata {
    ...
    public:
        ...
        IndexedValue(const Z ind, const T v) :
    ...
    void Correlations(const size_t nr, const size_t nv,
    ...
    bool Normalize_Array(const size_t nRows, const size_t nCols, const std::vector<T, AllocT> &data,
    std::vector<W, AllocW> &oDest, std::vector<W, AllocW> &oWeights) {
        assert(nRows > 1);
        assert(nCols > 0);
    ...
    template <typename T, class AllocT, typename W, class AllocW>
    bool Compute_Distances(const size_t nRows, 
            const size_t nCols, const std::vector<T, AllocT> &data,
    std::vector<W, AllocW> &oDest,
    std::vector<W, AllocW> &oRowDist, std::vector<W, AllocW> &oColDist) {
        ...
        for (size_t irow1 = 0; irow1 < nRows; ++irow1) {
            ...
            for (size_t irow2 = 0; irow2 < irow1; ++irow2) {
                ...
                double s = 0;
                for (size_t icol = 0; icol < nCols; ++icol) {
                    double x = (double) oDest[gap1 + icol] - (double) oDest[gap2 + icol];
        ...
        for (size_t icol1 = 0; icol1 < nCols; ++icol1) {
            ...
            for (size_t icol2 = 0; icol2 < icol1; ++icol2) {
                ...
                for (size_t irow = 0; irow < nRows; ++irow) {
                    ...
                    double x = (double) oDest[gap + icol1] - (double) oDest[gap + icol2];
                    s += x * x;
                }// irow
